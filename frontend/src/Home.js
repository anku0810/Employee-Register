...
function Home() {
    ...
    const handleSubmit = (event) =>{
        ...
        axios.post('http://localhost:3011/',{Emp_name, DOB, Age, Department,Designation,Salary,Address,Phone_No})
        .then(res =>{
            navigate('/');
    ...
    useEffect(()=>{
        axios.get('http://localhost:3011/')
        .then(res => setData(res.data))
        .catch(err => console.log(err));   
    })
    const handleDelete=(Emp_name)=>{
        axios.delete('http://localhost:3011/'+ Emp_name)
        .then(res => navigate('/'))
        .catch(err => console.log(err)); 
    }
  return (
        ...
        <div className='bg-gradient-to-r from-indigo-200 via-red-200 to-yellow-100 container mt-5'>
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className='inputs'>
                            ...
                            <div classname='inputs'>
                                <input type='number' placeholder='Enter Phone Number:' onChange={e => setPhone_No(e.target.value)}required/>
                            </div>
                        </div>
